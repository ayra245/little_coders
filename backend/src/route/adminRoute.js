...
router.get("/dashboard-stats", auth, verifyRole(["admin"]), async (req, res) => {
  try {
    ...
    const recentUsers = await User.find().sort({ createdAt: -1 }).limit(5).select("name email role updatedAt");
    const recentLessons = await Lesson.find().sort({ createdAt: -1 }).limit(5).select("name updatedAt");
